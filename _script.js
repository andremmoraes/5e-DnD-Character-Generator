...
var skills = ["Acrobacia","Adestrar Animais","Arcanismo","Atletismo","Enganação","História","Intuição","Intimidação","Investigação","Medicina","Natureza","Percepção"," Atuação","Persuasão","Religião","Prestidigitação","Furtividade","Sobrevivência"];
...
var armors = ["Escudo","Acolchoado","Armadura de Couro","Couro Batido","Gibão","Camisão de Cota de Malha","Brunea","Peitoral de aço","Meia armadura","Loriga","Cota de malha", "Cota de talas","Armadura de Batalha"];
var armorAC = [2,11,11,12,12,13,14,14,15,14,16,17,18];
var weapons = ["Adaga","Maça","Lança","Machadinha","Javelin","Besta leve","Besta pesada","Besta de mão","Arco curto","Arco longo","Martelo de arremesso","Machado de batalha","Greatsword","Halberd","Longsword","Morning Star","Rapier","Scimitar","Shortsword","Warhammer","Great Axe","Dart","Quarterstaff", "Unarmed Strike","Sling"];
...
var wpType = ["perfuração", "concussão", "perfuração", "corte", "perfuração", "perfuração","perfuração","perfuração","perfuração","perfuração", "concussão", "corte", "corte", "corte", "corte", "perfuração", "perfuração", "corte", "perfuração", "concussão", "corte","perfuração","concussão","concussão","concussão"];
...
var eldritch = ["Agonizing Blast","Armor of Shadows","Beast Speech","Beguiling Influence","Devil's Sight","Eldritch Sight","Eldritch Lança","Eyes of the Rune Keeper","Fiendish Vigor","Gaze of Two Minds","Mask of Many Faces","Misty Visions","Repelling Blast","Thief of Five Fates"];
var tome = false;
var waTome = [];
var waTomeRituals = [];
...
var enLangs = ["","","Celestial","","Draconico","Primordial","Silvestre","","Gigante","","","",""];
...
//determine passive perception
	...
	output += "Atributos:   &nbsp; FOR:  " + abil[0] + 
	" (" + showPlus( mod[0] ) + ") &nbsp; DEX:  " + abil[1] + " (" + showPlus( mod[1] ) + ") &nbsp; CON: " + abil[2]  + " ("+ showPlus( mod[2] ) +
		") &nbsp; INT: " + abil[3] + " ("+showPlus ( mod[3] )+") &nbsp;  SAB: " + abil[4] + " ("+showPlus ( mod[4] )+") &nbsp; CAR: " + abil[5] + " ("+showPlus ( mod[5] )+
		")<br>Modificadores de Atributos:  &nbsp; FOR:  " + showPlus( saves[0] ) + " &nbsp; DEX:  " + showPlus( saves[1] ) + "  &nbsp; CON: " +  
		showPlus( saves[2] ) + " &nbsp; INT: " + showPlus ( saves[3] )+" &nbsp;  SAB: " + showPlus ( saves[4] )+" &nbsp; CAR: " +showPlus ( saves[5] )+
		"<br>Iniciativa:  " + showPlus( initiative ) + " &nbsp; Deslocamento:  " + speed;
		...
		output += " &nbsp; Precepção passiva:  " + passive +" &nbsp; Tamanho:  " + size+" &nbsp; Bonus de Proeficiencia:  " + showPlus(prof) + " &nbsp; Genero:";
...
//	ra=1;
	...
	case (1):
		...
		elemType = tempAr[1];
		breathType = tempAr[2];
		abOutput += "Ancestral Draconico (" + dragType +  "), Breath Weapon (" + elemType + " damage in a "+breathType+"), Resistencia a dano (" + elemType + ")"; 
		if (gender==2) {
	...
	// eldritch knight
		...
		switch (fiStyle) {
			...
			case 2:
				...
				break;
			case 3:
				temp = "Lutar com arma grande";
				break;
			case 4:
				temp = "Proteção";
				break;
			case 5:
				temp = "Lutar com duas armas";
				break;
		}

		abOutput += "Estilo de combate ("+temp+"), Retomar o fôlego";				

		if (level > 1)
			abOutput += ", Pulso de ação";				

		if (level > 2) {
			abOutput += ", Arquetipo Marcial (Eldritch Knight), Spellcasting, Weapon Bond";				
...
//				temp = "Great Weapon Fighting"; doesn't work well for a finesse fighter
		...
		if (level > 2) {
			...
			if (x < 0.5) {
				classtxt += " (Campeão)";
				abOutput += ", Arquetipo Marcial (Campeão), Critico Aprimorado(19-20)";				
				if (level > 6) {
				...
				abOutput += ", Arquetipo Marcial (Battle Master), "+y+" Superiority Dice (d"+x+"), "+z+" Maneuvers ";
				pickManeuvers(z);
	...
	case 7:
		...
		if (level > 2) {
			...
			if (x < 0.5) {
				...
				classtxt += " (Champion)";
				abOutput += ", Arquetipo Marcial (Champion), Critico Aprimorado(19-20)";				
				if (level > 6) {
					abOutput += ", Remarkable Athlete";									
				}
	...
	case 8:
		...
		if ((level >5) && (x==0)) {
			abOutput += ", Wholeness of Body (" + (level*3) + " pontos de vida)";				
		}
		if ((level >5) && (x==1)) {
		...
		switch (soOrigin) {
			case 0:
				...
				if (racetxt != "Dragonborn") {
					...
					dragType = tempAr[0];
					elemType = tempAr[1];
					abOutput += ", Ancestral Draconico (" + dragType + ", " + elemType + " damage)";
				}
	...
	case 14:
		...
		if (level > 2) {
			...
			switch (x) {
				case 0:
					...
					if (waPatron ==0) {
						if (Math.random() < 0.5)
							familiar = "Sprite &mdash; 2 hit points, 15 AC, <i>Heart Sight, Invisibility</i>, Longsword +2 para acertar (1 dmg), Shortbow +6 para acertar (1 dmg, check if poisoned) Range 40/160";
						else 
							familiar = "Pseudodragon &mdash; 7 hit points, 13 AC, <i>Keen Senses, Magic Resistance, Limited Telepathy</i>, Bite +4 para acertar (1d4 +2 dmg), Sting +4 para acertar (1d4 +2 dmg, check if poisoned)";
					}
					if (waPatron ==1) {
						if (Math.random() < 0.5)
							familiar = "Quasit &mdash; 7 hit points, 13 AC, <i>Shapechanger, Magic Resistance, Scare, Invisibility,</i> Claws +4 para acertar (1d4+3 dmg, check if poisoned)";
						else 
							familiar = "Imp  &mdash; 10 hit points, 13 AC, <i>Shapechanger, Devil's Sight, Magic Resistance, Invisibility</i>, Sting +5 para acertar (1d4 +3 dmg, check for 3d6 poison dmg)";
					}
					if (waPatron ==2) {
		...
		if (level > 1) {
			...
			tempAr = pickEldritch();
				...
				if (tempAr.indexOf("Eldritch Lança") > -1) {
					eblast = true;
				}	
...
if (level > 3) {
	abOutput += ", Aprimeoramento de atributo (4º";
	console.log("4º Nivel ability bonus +2");
	x=2;
	
	if ((cl==5) || (cl==6) || (cl==7)) {
		if (level > 7) {
			abOutput +=", 6º and 8º níveis";
		} else if (level > 5) {
			abOutput +=" and 6º níveis";
		} else {
			abOutput +=" nivel";			
		}
	} else if ((cl==11) || (cl==12)) {
		if (level >9) {
			abOutput +=", 8th and 10º Nivels";
		} else if (level >7) {
			abOutput +=" and 8º Nivels";			
		} else {
			abOutput +=" level";
		}
	} else {
...
//bgOutput += "  D12:"+(D12())+"<br>";
//bgOutput += "  D20:"+(D20())+"<br>";
//bgOutput += "  D100:"+(D100())+"<br>";
//bgOutput += "  D100-2:"+(D100_2())+"<br>";


//bgOutput += antecedente+"<br>";
...
/* bgOutput += '<b style="font-size: large;">ANTECEDENTE:'+antecedente[antec][0]+'</b><br>'+
'<span class="bg">Personalidade:</span>('+(nper+1)+')'+antecedente[antec][7][nper]+'<br>'+
'<span class="bg">Ideal:</span>('+(nide+1)+')'+ antecedente[antec][8][nide]+'<br>'+
'<span class="bg">Vinculo:</span>('+(nvin+1)+')'+ antecedente[antec][9][nvin]+'<br>'+
'<span class="bg">Fraqueza:</span>('+(nfra+1)+')'+ antecedente[antec][10][nfra]+'<br>'+
'<br>'; */
/*
bgOutput += bgText;
...
if (cl==15) {
	...
	// 3rd and 4º Nivels add more 2nd level spells
	if (level >2) 
		sp2 += ((level-2) * 2);
...
// check if we already know spell
	...
	// polymorph automatically in book for 10º Nivel transmutation wizards
}
...
if (ss1 > 0) {
	...
	if (ss4 > 0) 
		spOutput += ", " + ss4+ " fourº Nivel";
	if (ss5 > 0) 
		spOutput += ", " + ss5+ " fifº Nivel";
	}
	if (cl==14) {
		if (level < 3)
			spOutput += " first level";
			...
			//20% rare
				...
				if ((Math.random() < 0.25) && (magicArmor==0) && (myAr.length > 0)) {
					...
					if (upgradeVeryRare() == true) {
						...
						maOutput += " (Resistencia a " + ringResistance[i] + " dano)";	
					}
					whichMagicArmor = q;
					z--;
...
if (companion > -1) {
	...
	eqOutput += "<br><b>Ranger's Companion:</b>  " + companions[companion] + " &mdash; " + (companionshp[companion] * 1) + " hit points, "+((companionsAC[companion] * 1)+ (prof*1)) + " AC, " + (companionsStats[companion]) + ((companionsAttack1[companion] * 1)+prof) + " para acertar (" +(companionsDamage1[companion]) +"+"+(companionsDamageBonus[companion]+prof) + " dmg" +	companionsDamageExtra[companion] + ")";
	if (companionsAttackText2[companion].length > 0) {
		eqOutput += ", " + companionsAttackText2[companion] + " +" + ((companionsAttack2[companion] * 1)+prof) + " para acertar (" +(companionsAttackDamage2[companion]) +"+"+(companionsDamageBonus2[companion]+prof) + " dmg)";
	}
...
for (x=0;x < myWp.length;x++) {
	...
	if (wpRange[i] != 2) {
	   // if it's a finesse weapon and DEX is higher than STR then use dex bonus
		...
		if ((fiStyle==2) && (i != 12) && (i != 13) && (i != 20) && (wpNum[i] < 1))
			z=2;
		wpOutput += " (" + showPlus( mod[temp]+prof+y ) +  " para acertar), Dano: " +wpDam[i]+ showPlus( mod[temp]+z+y ) + " " + wpType[i] + "<br>";	
	}	
	...
	if (wpRange[i] > 1) {
		...
		// +2 para acertar if fighter's fighting style is archery
		z=0;
		...
		wrOutput += " (" + showPlus( y+z+prof+q ) +  " para acertar) " +tempTxt+ showPlus( y+q ) + " " + wpType[i] + " de dano &nbsp; Distancia: "+wpDistance[i];
		...
		if ((i == 5) || (i == 6) || (i == 7)) {
			...
			if (((magicArrowPlus > 0) && (magicArrowType.indexOf("olt") > -1)) && (magicArrowDisplay==false)) {
				wrOutput += "</li><li>+" + magicArrowPlus + " " + magicArrowType +" (" + magicArrows + ") (" + showPlus( y+z+prof+q+magicArrowPlus ) +  " para acertar) "+tempTxt+ showPlus( y+q+magicArrowPlus ) + " " + wpType[i] + " de dano";
...
if (wrOutput.length >0)
	wpOutput += "<b>Armas de longa distancia:</b>  <br>" + wrOutput;
...
if (myAr[0] == "Escudo" && (myAr.length > 1)) {
	myAr.reverse();
...
arOutput+= "<b>Pontos de Vida: " + hp + "</b> &nbsp; Dados de Vida:  " + level + "d" + hd +"<br><b>Armadura:  ";
...
if (toOutput != "") {
	...
	abOutput =  "<b>Linguas:</b>  <span id=\"la\"></span> &nbsp; &nbsp; <b>Outras proficiencias:</b>" + toOutput + "<br>" + abOutput;
} else {
	abOutput =  "<b>Lingua:</b>  <span id=\"la\"></span><br>"+ abOutput;
}
...
//7º Nivel champions get a bonus too
...
/*
var background_array = Array('ACÓLITO', 'ARTESÃO DE GUILDA', 'ARTISTA', 'CHARLATÃO', 'CRIMINOSO', 'EREMITA', 'FORASTEIRO', 'HERÓI DO POVO', 'MARINHEIRO', 'MENINO DE RUA', 'NOBRE', 'SÁBIO', 'SOLDADO');
...
switch (bg) {
	...
	case(1):
		...
		myLifestyle = 3;
		bgOutput += "</b>"
		if (eqOutput.indexOf("Simbolo Sagrado") == -1) 
			eqOutput += ", Simbolo Sagrado";

		eqOutput += ", livro de preces ou uma roda de preces, 5 palitos de incenso, vestimentas, um conjunto de roupas comuns e uma algibeira contendo 15 po";
		abOutput += "";
		gold += 15;
	...
	case(7):
		...
		bgText += "<br><span class=\"bg\">Personalidade:</span> ";
		...
		while (i < 2) {
			...
			if (personality==1)
			bgText += "Eu julgo as pessoas por suas ações, não por suas palavras. ";
			if (personality==2)
			bgText += "Se alguém está em apuros, eu estou sempre pronto para ajudar. ";
			if (personality==3)
			bgText += "Quando eu fixo minha mente em algo, eu sigo esse caminho, não importa o que fique no caminho. ";
			if (personality==4)
			bgText += "Eu possuo um forte senso de justiça e sempre tento encontrar a solução mais equilibrada para as discussões. ";
			if (personality==5)
			bgText += "Eu confio em minhas habilidades e farei o que for necessário para que os outros confiem também. ";
			if (personality==6)
			bgText += "Pensar é para os outros, eu prefiro agir. ";
			if (personality==7)
			bgText += "Eu abuso de palavras longas na tentativa de soar inteligente. ";
			if (personality==8)
			bgText += "Eu me entedio fácil. Para onde devo ir para me encontrar com meu destino? ";
		...
		bgText += "<br><span class=\"bg\">Ideal:</span> ";
		if (ideal==1)
			bgText += "Respeito. As pessoas merecem ser tratadas com dignidade e respeito. (Bom)";
		if (ideal==2)
			bgText += "Justiça. Ninguém merece tratamento diferenciado perante a lei, muito menos estar acima dela. (Leal)";
		if (ideal==3)
			bgText += "Liberdade. Não pode haver permissão para tiranos oprimirem o povo. (Caótico)";
		if (ideal==4)
			bgText += "Força. Se eu ficar forte, eu posso pegar tudo o que eu quiser – o que eu desejar. (Mal)";
		if (ideal==5)
			bgText += "Sinceridade. Não há nada de bom em fingir ser algo que não sou. (Neutro)";
		if (ideal==6)
			bgText += "Destino. Nada, nem ninguém, pode me manter longe do meu chamado. (Qualquer)";
		

		bgText += "<br><span class=\"bg\">Vinculo:</span> ";
		if (bond==1)
			bgText += "Eu tenho família, embora não faça a mínima ideia de onde eles estão, espero encontrá-los um dia.";
		if (bond==2)
			bgText += "Eu trabalho a terra, eu amo a terra e eu vou defender a terra.";
		if (bond==3)
			bgText += "Um nobre orgulhoso me deu uma bela surra, e eu vou ter minha vingança em qualquer valentão que encontrar.";
		if (bond==4)
			bgText += "Minhas ferramentas são símbolo de minha vida passada, eu as carregarei para nunca me esquecer de minhas origens.";
		if (bond==5)
			bgText += "Eu devo proteger aqueles que não podem se defender.";
		if (bond==6)
			bgText += "Gostaria que meu amor viesse comigo para seguir meu destino.";

		bgText += "<br><span class=\"bg\">Fraqueza:</span> ";
		if (flaw==1)
			bgText += "O tirano que comanda minha terra não vai parar até ver meu cadáver.";
		if (flaw==2)
			bgText += "Eu estou convencido sobre o significado do meu destino, e cego aos riscos e falhas.";
		if (flaw==3)
			bgText += "As pessoas que me conhecem desde criança sabem de um vergonhoso segredo meu, eu não poderei voltar para casa nunca.";
		if (flaw==4)
			bgText += "Eu tenho uma fraqueza pelos vícios da cidade, especialmente a bebedeira.";
		if (flaw==5)
			bgText += "Secretamente, eu acredito que as coisas estariam melhores se algum tirano comandasse a região.";
		if (flaw==6)
			bgText += "Eu tenho dificuldades em confiar em meus aliados.";
